        ...
        <section class="sub-section-row">
            ...
            <div class="sub-column main-photo-wrap photo-wrap">
                <dropzone
                    ...
                    v-on:vdropzone-success="mainImageUploadSuccess"
        ...
        <section class="sub-section-row">
            ...
            <div class="section-description sub-column">
                <section class="description-detail">
                    ...
                    <p>A handful of pictures can say a lot, right? This is your chance to show your spot off to the world. You can upload up to 20 photos, so don’t be shy. <small>(These are optional)</small></p>
                </section>
            </div>
            ...
            <div class="sub-column secondary-photos-wrap photo-wrap">
                <dropzone
                    ...
                    :useCustomSlot="true"
                    v-on:vdropzone-thumbnail="thumbnail"
                    v-on:vdropzone-success="secondaryImageUploadSuccess"
                    v-on:vdropzone-removed-file="imageRemoved"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            addInitPhotos() {
                if (!this.initPhotos || !this.initPhotos.length) return false;
                let self = this;
                this.initPhotos.forEach(photo => {
                    self.$emit('add-photo',{id: photo.id});
                });

                let mainPhoto = this.initPhotos[0];
                this.$refs.mainDropzone.manuallyAddFile(mainPhoto, mainPhoto.url);
                                
                let dz = this.$refs.secondaryDropzone;
                let otherPhotos = this.initPhotos.slice(1);
                otherPhotos.forEach((photo) => {
                    dz.manuallyAddFile(photo, photo.thumbnail);
                });
            },
            ...
            secondaryImageUploadSuccess(file, response) {
                file.id = response.media_id;
                this.$emit('add-photo', {
                    isMain : false,
                    id : response.media_id
            ...
            imageRemoved(file) {
                this.$emit('remove-photo', file.id);
            }
        },
        computed: {
            csrf() {
