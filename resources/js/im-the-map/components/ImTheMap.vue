  ...
  >
    ...
    <header>
      ...
      <section class="logo-and-menu">
        ...
        <div
          class="logo"
          @click.prevent="homePane()"
        ></div>

        <div
          class="bun"
          :class="{active: mobileNavIsVisible}"
          @click.prevent="mobileNavIsVisible = !mobileNavIsVisible"
        >
      ...
      <nav>
        <a
          href="#"
          @click.prevent="toggleGlobalCalendarDropdown"
        >When?</a>
        <a
          href="#"
          @click.prevent="showAboutModal"
        ...
        <a
          href="#"
          @click.prevent="showContactDetails"
        >Contact</a>
        <a
          href="#"
          @click.prevent="showSubmitPropertyModal"
        >Owner Sign Up</a>
        <a
          href="#"
      ...
      <section
        v-if="globalCalendarDropdownIsVisible"
        id="global-calendar-dropdown"
      >
    ...
    <ul
      ...
      :class="{open: styleSwitcherIsOpen}"
    >
      <li
        class="default"
        :class="{active: (mapStyle === 'light-v10')}"
        @click.prevent="changeMapStyle('light-v10')"
      >
        <svg
          width="16"
          height="16"
      ...
      <li
        class="streets"
        :class="{active: (mapStyle === 'streets-v11')}"
        @click.prevent="changeMapStyle('streets-v11')"
      >
        <svg
          width="16"
          height="16"
      ...
      <li
        class="satellite"
        :class="{active: (mapStyle === 'satellite-streets-v9')}"
        @click.prevent="changeMapStyle('satellite-streets-v9')"
      >
        <svg
          width="16"
          height="16"
        ...
        >
          <path
            d="M4.64976 5.58781C5.05047 5.98464 5.69698 5.9816 6.09393 5.581L7.05262 4.61414C7.2434 4.42168 7.34987 4.16129 7.34859 3.8903C7.34732 3.61932 7.2384 3.35994 7.04581 3.16929L4.14454 0.29458C3.74326 -0.100571 3.0983 -0.0978339 2.70038 0.300708L1.74169 1.26825C1.34485 1.66896 1.3479 2.31547 1.7485 2.71242L4.64976 5.58781Z"
            fill="#29304C"
          ></path>
          <path
            d="M15.6993 11.8696L12.7966 8.99416C12.395 8.59957 11.7504 8.60261 11.3525 9.00097L10.3938 9.96851C9.99695 10.3692 10 11.0157 10.4006 11.4127L13.3032 14.2881C13.7039 14.6849 14.3504 14.6819 14.7474 14.2813L15.7054 13.311C16.1006 12.9108 16.0979 12.2664 15.6993 11.8696Z"
            fill="#29304C"
          ></path>
          <path
            d="M13.897 3.44986H13.9018C14.6677 3.44855 15.2877 2.82693 15.287 2.06102C15.2864 1.29511 14.6653 0.674561 13.8994 0.674561C13.1335 0.674561 12.5124 1.29511 12.5117 2.06102C12.5111 2.82693 13.1311 3.44855 13.897 3.44986Z"
            fill="#29304C"
          ></path>
          <path
            d="M4.76628 12.5964C4.01419 12.5964 3.4045 11.9867 3.4045 11.2346C3.4045 10.8586 3.09966 10.5537 2.72361 10.5537C2.34757 10.5537 2.04272 10.8586 2.04272 11.2346C2.04272 12.7388 3.2621 13.9582 4.76628 13.9582C5.14232 13.9582 5.44717 13.6533 5.44717 13.2773C5.44717 12.9012 5.14232 12.5964 4.76628 12.5964Z"
            fill="#29304C"
          ></path>
          <path
            d="M4.42578 14.6389C2.73358 14.6389 1.36178 13.2671 1.36178 11.5749C1.36178 11.1989 1.05693 10.894 0.680889 10.894C0.304844 10.894 0 11.1989 0 11.5749C0.00300129 14.018 1.98273 15.9977 4.42578 16.0007C4.80182 16.0007 5.10666 15.6959 5.10666 15.3198C5.10666 14.9438 4.80182 14.6389 4.42578 14.6389Z"
            fill="#29304C"
          ></path>
          <path
            d="M12.0954 3.87947C11.2941 3.08561 10.0011 3.0914 9.20702 3.8924L6.1546 6.97479C6.02773 7.10349 5.95723 7.27735 5.95864 7.45807C5.96004 7.63878 6.03323 7.81153 6.16209 7.93824L8.09649 9.85495C8.36361 10.119 8.7941 10.1169 9.05859 9.85018L12.111 6.7678C12.9041 5.96582 12.8971 4.6728 12.0954 3.87947Z"
            fill="#29304C"
          ></path>
          <path
            d="M3.72177 8.68112C3.52255 8.77711 3.38235 8.96387 3.3458 9.18197C3.30925 9.40007 3.38089 9.62233 3.53793 9.77803L6.27442 12.49C6.43138 12.6459 6.65426 12.7157 6.8721 12.6774C7.08994 12.6391 7.2756 12.4974 7.36997 12.2973C7.86374 11.2534 7.64545 10.0115 6.82526 9.1985C6.00508 8.38551 4.76133 8.17816 3.72177 8.68112Z"
            fill="#29304C"
          ></path>
        </svg>
      </li>
  ...
  methods: {
    ...
    applyFilters() {
      ...
      if (this.hasActiveFilters) {
        ...
        let filteredData = this.geoJson.features.filter(feature => {
          return (
    ...
    markerClicked(feature) {
      let self = this;
      this.$store
        .dispatch("triggerNewActiveSpot", feature.id)
        .then(response => {
          self.newActiveSpot(feature);
        });
    },
    gotoCoords(coords) {
      this.resizeMap();
    ...
    showMarkerForSpot(feature) {
      // AB: bad code, find out why this is being turned into a string
    ...
    hideHoverMarker() {
      if (
        !(
          Object.entries(this.hoverMarker).length === 0 &&
          this.hoverMarker.constructor === Object
        )
      ) {
    ...
    showContactDetails() {
      this.$store.commit("setShowContact", !this.$store.state.showContact);
    },
    gotoLogin() {
    ...
    updateMarkers() {
      ...
      features.forEach(feature => {
        ...
        if (self.map.getZoom() <= 8 || props.cluster) return;
        var id = props.id;
        ...
        if (!marker) {
          ...
          el.className = "marker";
          el.innerHTML =
            '<div class="spot-marker"><div class="image" style="background-image:url(' +
            feature.properties.photo +
            ')"></div><section><span class="price">$' +
            feature.properties.price +
            '</span><span class="beds">' +
            feature.properties.beds +
            '</span><span class="baths">' +
            feature.properties.baths +
            "</span></section></div>";
        ...
        if (!self.markersOnScreen[id]) marker.addTo(self.map);
      });
    ...
    initData(geoJson) {
      ...
      this.map.on("sourcedata", () => {
        if (
          !self.mapIsLoaded &&
          self.map.getSource("places") &&
          self.map.isSourceLoaded("places")
        ) {
      ...
      this.map.on("styledata", () => {
        if (
          !this.map.getSource("places") &&
          !this.map.isSourceLoaded("places")
        ) {
          this.map.addSource("places", {
            type: "geojson",
            data: geoJson
